...
def gen_daily_data(name, lat, lon, t_start, t_end):
    '''
    ...
    # Take daily averages of gas/particulate element and add to list
    element_list = []
    element_count = 0
    for i in range(df_size):
        element_count+=ls[i]['components']['pm2_5']
        # Average for each day
        if (i%24 == 0):   
            element_daily = round(element_count/24, 5)
            element_list.append(element_daily)
            element_count = 0
    ...
    while (curr_index < curr_index+60):
        ...
        with open('C:\\github_repos\\Universal-Embeddings\\data\\geocoded-cities-master.csv', 'a', newline='') as f_open:
            ...
            for i in range(curr_index, curr_index+60):
                ...
                entry = gen_daily_data(name=city_name, lat=city_lat, lon=city_lon, t_start=T_START, t_end=T_END)
                city_info+=entry
